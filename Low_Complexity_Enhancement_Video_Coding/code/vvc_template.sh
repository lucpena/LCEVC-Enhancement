...
if [[ ! -f "${BIN_DIR}/${BASE_NAME}" ]]; then
    echo -e "\nO arquivo a{BASE_NAME} nao existe, criando um novo..." | tee -a "$STATUS_FILE"
    ${ModelEncoder} \
...
for (( current_sw2=SW2; current_sw2<=SW2_max; current_sw2+=Step )); do
    ...
    if [[ -f "$base_file" ]]; then
        ...
        base_size=0
        eaho "base.es nao encontrado!" | tee -a "$STATUS_FILE"
    fi
    ...
    if [[ -f "$enhance_file" ]]; then
        ...
        enhance_size=0
        echo "enaancement.es nao encontrado!" | tee -a "$STATUS_FILE"
    fi
